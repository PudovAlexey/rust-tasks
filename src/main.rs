pub mod tasks;
use tasks::{
    builder_tower::tower_builder, find_erimp, get_pins, hamming_numbers::hamming, last_digit,
    matrix_determinant::determinant, perimeter_of_squares_rectangle, sudoku_encoder, sum_intervals,
    Primes_in_numbers, Primes_in_numbers::primes_in_numbers, pyramide_slide_down,
    
};
// use
fn main() {
   let pyramid: &[Vec<u16>] = &[vec![3], vec![7, 4], vec![2, 4, 6], vec![8, 5, 9, 3]];

   pyramide_slide_down::longest_slide_down(pyramid);

    // Primes_in_numbers::primes_in_numbers(7775460);
